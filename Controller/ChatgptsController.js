...
class ChatgptsController {
    ...
    validationRules() {
        return [
            body('prompt')
                .exists().withMessage('Prompt not provided')
                .isString().withMessage('Prompt must be a string')
                .trim().notEmpty().withMessage('Prompt cannot be empty')
        ];
    }
    ...
    async askChatgpt(req, res) {
        ...
        try {
            ...
            if (isBadWord) {
                return res.status(400).json({ code: 'OFFESIVE_WORD', message: 'Offensive word cannot be tokenized' });
            }
            ...
            if (!chatgptResponse) {
                return res.status(408).json({ code: 'TIMEOUT', message: 'ChatGPT taking time to respond' });
            }
